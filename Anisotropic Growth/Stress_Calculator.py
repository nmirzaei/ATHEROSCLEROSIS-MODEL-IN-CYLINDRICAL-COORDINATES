...
while counter<=M:
 ...
 if counter%Skip==0:   
    #Read the displacement saved from the growth code
    u = Function(V,'/media/user1/Seagate Backup Plus Drive/Dropbox/Research/Fenics codes Paper 1/Cylindrical_Strip_Paper/Anisotropic_Growth/Two Field Anisotrpoic/Anisotropic/displacement/u%d.xml' %(counter))
    ######################################################################

    #Read the growth parameters
...
#    psi_i = ((mu_i/2)*(Ie_i - 3)+((mu_i*nu_i)/(1-2*nu_i))*(J_i-1)**2- mu_i*ln(J_i)+(eta_i/beta_i)*(exp(beta_i*(rho_i*(I4_i-1)**2+(1-rho_i)*(Ie_i-3)**2))-1))
...
#    psi_a = ((mu_a/2)*(Ie_a - 3)+((mu_a*nu_a)/(1-2*nu_a))*(J_a-1)**2- mu_a*ln(J_a)+(eta_a/beta_a)*(exp(beta_a*(rho_a*(I4_a-1)**2+(1-rho_a)*(Ie_a-3)**2))-1))
     ######################################################################   
#    sig_i = (2/J_i)*dot(Fe_i,dot(diff(psi_i,Ce_i),Fe_i.T))
#    sig_m = (2/J_m)*dot(Fe_m,dot(diff(psi_m,Ce_m),Fe_m.T))
#    sig_a = (2/J_a)*dot(Fe_a,dot(diff(psi_a,Ce_a),Fe_a.T))
 ...
 ####################################################################### 
    if counter==M2:
        #Sending to this function for ploting process
        stress(u,t,Sig_i,Sig_m,Sig_a,vtkfile1,vtkfile2,vtkfile3) 

    #Sending to this function for finding the maxima 
...
#Saving the maxima for each direction and each layer in csv files
import csv
