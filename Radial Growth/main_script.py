...
vtkfile11 = File("R_Growth/Vol_rec.pvd")
vtkfile22 = File("R_Growth/bnd_rec.pvd")
vtkfile33 = File("R_Growth/domain_rec.pvd")
vtkfile55 = File("R_Growth/deformmed_N.pvd")
######################################################################
...
#making sure we start saving results after the pressure is imposed (if there is any pressure). Also making sure we save results every 10 steps    
 ...
 if sigma>sigmax:
     ...
     File("R_Growth/displacement/u%d.xml" %(counter))<<u  #Save the displacements for later stress computations
     epsilon+=deps
     t+=dt
     counter+=1
     ID+=1
