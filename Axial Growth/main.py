...
V = VectorFunctionSpace(mesh, 'P', 1)  #degree 2 vector space to eliminate locking
VV = FunctionSpace(mesh,'DG',0)
SS = FunctionSpace(mesh,'P',1)
T = TensorFunctionSpace(mesh,'DG',1,shape=(3,3))
...
ginv_i = Expression((('1','0','0'),('0','1/(1+epsilon*exp(-a*(x[1]-1.68)*(x[1]-1.68)))','0'),('0','0','1')),degree=1,epsilon=0.0,a=14.2857142857)
...
#Files for saving the domain evolution
vtkfile11 = File("Z_Growth/Vol_rec.pvd")
vtkfile22 = File("Z_Growth/bnd_rec.pvd")
vtkfile33 = File("Z_Growth/domain_rec.pvd")
vtkfile55 = File("Z_Growth/deformmed_N.pvd")
######################################################################
...
#making sure we start saving results after the pressure is imposed (if there is any pressure). Also making sure we save results every 10 steps
 ...
 if sigma>sigmax:
     ...
     File("Z_Growth/displacement/u%d.xml" %(counter))<<u  #Save the displacements for later stress computations
     epsilon+=deps
     t+=dt
     counter+=1
     ID+=1
