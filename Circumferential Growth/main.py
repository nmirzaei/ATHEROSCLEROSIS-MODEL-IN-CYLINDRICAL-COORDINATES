...
os.chdir("Lustre storage location")
######################################################################


# define function space
V = VectorFunctionSpace(mesh, 'P', 2)  #degree 2 vector space to eliminate locking
VV = FunctionSpace(mesh,'DG',0)
SS = FunctionSpace(mesh,'P',1)
T = TensorFunctionSpace(mesh,'DG',0,shape=(3,3))
...
vtkfile11 = File("Theta_Growth/Vol_rec.pvd")
vtkfile22 = File("Theta_Growth/bnd_rec.pvd")
vtkfile33 = File("Theta_Growth/domain_rec.pvd")
vtkfile55 = File("Theta_Growth/deformmed_N.pvd")
######################################################################
...
#making sure we start saving results after the pressure is imposed (if there is any pressure). Also making sure we save results every 10 steps
 ...
 if sigma>sigmax:
     ...
     File("Theta_Growth/displacement/u%d.xml" %(counter))<<u  #Save the displacements for later stress computations
     epsilon+=deps
     t+=dt
     counter+=1
     ID+=1
