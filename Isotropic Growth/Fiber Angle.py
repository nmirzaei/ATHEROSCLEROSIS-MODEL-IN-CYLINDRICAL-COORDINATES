...
#Extracts the value of a function in a specific domain and sets the rest of its values equal to 0
...
deps, dsig, dt= 0.001, 16.0, 0.001
epsmax, sigmax = 0.001, 16.0
j=0
k=0
ID=0
Angles_i = File('local location/Angles_i.pvd')
Angles_m = File('local location/Angles_m.pvd')
Angles_a = File('local location/Angles_a.pvd')
######################################################################
...
while counter<=M:
 ...
 if counter%Skip==0:
    ...
    if counter!=0:
        #calcualte the isotropic growth parameter
