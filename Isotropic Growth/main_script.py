...
vtkfile11 = File("Isotropic_Growth_1/Vol_rec.pvd")
vtkfile22 = File("Isotropic_Growth_1/bnd_rec.pvd")
vtkfile33 = File("Isotropic_Growth_1/domain_rec.pvd")
vtkfile55 = File("Isotropic_Growth_1/deformmed_N.pvd")
#####################################################################
...
while int(epsilon*1000)<=int(epsmax*1000):
 ...
 #Read the parameters calculated by Growth Parameters codes
 if counter!=0:
     a1, b1, c1 = Reader(counter)
     #Calculate the isotropic parameter that provides the same exact Jacobian as th anisotropic case
...
######################################################################
 ...
 #Growth in the intima with the isotropic parameter
...
######################################################################
 ...
 if sigma>sigmax:
     ...
     File("Isotropic_Growth_1/displacement/u%d.xml" %(counter))<<project(u,V) #Save the displacements for later process
     ######################################################################
